...
// Сохранение завершения сортировки
exports.completeSort = async (req, res) => {
    const { palletNumber, items } = req.body;

    if (!palletNumber || !items) {
        return res.status(400).json({ message: 'Необходимо указать palletNumber и items' });
    }

    const query = `
        INSERT INTO completed_sort (pallet_number, item_ids, created_at)
        VALUES (@palletNumber, @items, GETDATE())
    `;

    try {
        await executeQuery(query, { palletNumber, items: items.join(',') });
        res.status(200).json({ message: 'Сортировка завершена' });
    } catch (err) {
